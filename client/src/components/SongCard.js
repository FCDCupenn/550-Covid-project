...
export default function SongCard({ songId, handleClose }) {
  ...
  useEffect(() => {
    ...
    //     })
      ...
      .then(res => res.json())
      .then(song => {
        // console.log('Fetched song:', song); // Log the song data
        setSongData(song);
        fetch(`http://${config.server_host}:${config.server_port}/album/${song.album_id}`)
        .then(res => res.json())
        .then(album => {
  ...
  const handleGraphChange = () => {
    // console.log('Current barRadar value:', barRadar); // Check current state
